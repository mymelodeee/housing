...
export interface LiveListingGroup {
  key: string
  complexName: string
  address: string | null
  ...
  minSalePrice: number
  maxSalePrice: number
  entries: RegionalListing[]
}

export function groupLiveListingsByComplex(entries: RegionalListing[]): LiveListingGroup[] {
  const groups = new Map<string, LiveListingGroup>()
