...
- docs 디렉토리, swagger/swagger.json 문서를 분석한 후 docs\7-execution-plan.md의 ${TASK_NUMBER}의 내용을 읽어와 분석한다.
- 기존 코드 분석 : 코드베이스(backend 디렉토리)의 코드를 분석한다.
- 기존 코드 분석과 docs 문서 분석은 병렬로 수행한다.
...
- 테스트가 완료되면 실행 계획 문서의 ${TASK_NUMBER}의 완료 조건의 체크박스를 체크한다.

### 반드시 준수할 것

- 모든 로깅 코드는 콘솔기반 로그를 사용하도록 개발한다.
- 주요 자격증명 정보는 환경변수를 이용하도록 한다.
