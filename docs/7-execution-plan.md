...
| v0.36 | 2026-09-08 | **대출 시뮬레이션 기준금리를 DB 기반 단일 출처로 전환(사용자 요청)**: 기존 `loan-limit.service.js`/`loan-scenario.service.js`에 하드코딩돼 있던 "우리은행 우리아파트론 4.37%·한국은행 가중평균금리 4.34%(2026.3월) 교차검증 4.5%" 가정을 폐기하고, 신규 `market_interest_rates` 단일 행 테이블(ERD 반영)에 저장된 **한국은행 금융기관 가중평균금리(예금은행 신규취급액 기준 주택담보대출)** 하나만을 기준금리로 쓰도록 변경(2026-09-08 확인값 연 4.48%, 2026년 7월 통계·2026-08-27 발표). `loan-limit.service.js`/`loan-scenario.service.js`는 금리를 내부 상수 대신 호출부가 넘기는 `annualInterestRate` 파라미터로 받는 순수함수로 변경하고, `complex-detail.service.js`/`listings.service.js`가 신규 `market-interest-rate.service.js`에서 조회해 주입한다. 안정적인 공개 API가 없어(ECOS API 키 미보유) 사람이 매월 발표를 확인해 반영하는 방식을 그대로 채택하되, `checked_at` 기준 30일 경과 시 `isStale=true`로 재조사 대상임이 API 응답(`interestRateMeta`, `GET /api/admin/market-interest-rate`)에 드러나도록 리모델링 stale-check 패턴을 재사용했다. 신규 `scripts/refresh-market-interest-rate.js --check`/`--apply`로 사람이 주기적으로 갱신한다. 백엔드 47개 스위트·579개 테스트 통과(기존에 알려진 무관한 리모델링 날짜-drift 실패 3건 제외) |
| v0.37 | 2026-09-08 | **단지 비교 화면 "정보 없음" 항목 자동 보강 감사·구현(사용자 요청)**: 먼저 서브에이전트로 비교 화면 데이터 흐름 전체를 감사(연식이 일부 단지에서 조회년도 2026으로 잘못 고정돼 있던 데이터 품질 결함 발견 등). 신규 공공API/키 없이 기존에 이미 승인된 API만 재사용해 3개 항목을 자동화했다. ① **강남 접근성**: 신규 `gangnam-accessibility.service.js`가 강남역 좌표(37.497942, 127.027621, 기존 geocoding 확인값 재사용)와 haversine 직선거리를 계산(외부 API 미사용, 실패 케이스 없음) — 기존에 도메인 문서(§4.1)에는 "자동 산정값"이라고만 적혀 있고 실제 구현이 없던 문서-코드 불일치를 해소. ② **상권**: 기존 소상공인시장진흥공단 상가정보 API(B553077, 유흥/학원가와 동일 키)를 재사용해 신규 `commercial-area.service.js`가 700m 반경 음식점/카페/마트·편의점/병원 대표 업종 건수를 집계("음식점 N · 카페 N · 마트/편의점 N · 병원 N" 형식, datago-api-prober로 업종코드 실측). ③ **연식(준공년도) 데이터 품질**: 이미 사용 중인 국토교통부 매매 실거래가 API(RTMSDataSvcAptTrade) 응답의 `buildYear` 필드에서 단지별 최빈값을 역산하는 `resolveCompletionYearFromTrades`를 `molit-price-history.service.js`에 추가하고, 일회성 보정 스크립트(`scripts/backfill-completion-year-from-molit.js --check`/`--apply`)로 실제 `housing` DB에서 15개 단지의 오류값(예: 명일지에스아파트 2026→1999)을 확인·반영. **부수적으로 이 API의 실측 일일 호출 한도 초과를 겪어**, `parseAptTradeXml`/`fetchMatchedTransactions`에 API 실패와 "0건 성공"을 구분하는 `hasApiError` 플래그를 추가(전체 실패 시 "실거래 이력 없음" 대신 "확인 필요" 반환) — 나머지 22개 단지는 한도 초기화 후 재실행 필요. ④ 비교 화면의 "연식"·"주변 재개발 정보" null 표시를 "정보 없음"에서 "확인 필요"로 정정(공식 데이터가 없다는 뜻이 아니라 조사 미완료라는 뜻이므로). **자동화하지 않은 항목**: 주변 일자리(국민연금공단/공정위/산업단지공단), 교통(K-apt/버스정류장), 공원(전국도시공원정보표준데이터 확인 결과 `tn_pubr_public_cty_park_info_api`로 별도 활용신청 필요) — 전부 신규 공공API 활용신청이 필요해 이번 범위에서 구현하지 않고 필요한 API/엔드포인트만 문서화. 재개발 정비사업 실데이터·재건축 추진현황 자동화는 전국 단일 공식 API가 없어 수동 curated 데이터가 필요하나 이번 세션에서는 착수하지 않음. 백엔드 53개 스위트·604개 테스트 중 기존에 알려진 무관한 리모델링 날짜-drift 실패 3건 제외 전부 통과, 프론트엔드 90개 파일·379개 테스트 전부 통과, `tsc --noEmit` 클린 |
| v0.38 | 2026-09-09 | **[대출/자금] 탭 중개보수·규제지역 신고 감사 및 수정(사용자 신고)**: 서브에이전트로 중개보수 계산·규제지역 판정 흐름 전체와 공식 법령/고시를 먼저 감사했다. **중개보수**: 아파트 매매 상한요율표(공인중개사법 시행규칙 별표1, 2026-08-28 시행) 자체는 이미 정확히 구현돼 있었고(가격 구간별 0.4~0.7% 동적 계산, "0.7% 고정"이 아님), 사용자가 우려한 문제는 실재하지 않았음을 확인했다 — 다만 감사 중 발견한 두 공백을 보완했다: ① **VAT 별도 고지 누락**: `calculateBrokerageFeeFromRate`에 `includeVat` 파라미터는 있었으나 호출부(`complex-detail.service.js`)가 넘기지 않아 항상 VAT 미포함으로 계산되면서도 화면에 안내가 없었다 — `brokerageFee.vatIncluded: false` 필드를 응답에 추가하고 `ComplexAcquisitionCostSection`에 "부가가치세 별도" 안내 문구를 추가(법제처 유권해석 확인: 법정 상한액에 VAT 미포함, 일반과세자는 별도 청구 가능). ② **오피스텔 요율 미구현**: `brokerage-fee.service.js`에 `calculateOfficetelBrokerageFee`(매매 0.5%/임대차 0.4% 단일요율, 공인중개사법 시행규칙 제20조제4항제1호·별표2, 국토교통부령 제1611호 2026-08-11 개정 확인)를 신규 추가했다 — 단, 이 서비스는 매물 유형(아파트/오피스텔) 구분 컬럼 자체가 없어 실제 오피스텔 매물을 취급하지 않으므로 요건 판정 로직 없이 계산 함수만 준비해 뒀다(문서화된 공백). **규제지역**: DB 구조 자체는 이미 조정대상지역·투기과열지구(`is_regulated_area`)와 토지거래허가구역(`is_land_transaction_permission_zone`, tri-state)이 분리돼 있어 "하나로 뭉침" 문제는 없었으나, **국토교통부공고 제2025-1219호(2025.10.20~2026.12.31 시행)로 서울 21개구+경기 12곳(성남 분당·수정·중원 포함)이 조정대상지역·투기과열지구·토지거래허가구역 3종으로 신규 지정된 이후에도 DB 데이터가 갱신되지 않은 결함**을 발견 — 서울 강동구 9곳, 성남 중원구 3곳·수정구 1곳·분당구 1곳 총 14개 단지의 두 컬럼을 모두 `true`로 정정(`housing` DB 직접 UPDATE). 규제 판정값이 `is_regulated_area`/`is_land_transaction_permission_zone` → `complex-detail.service.js`의 `effectiveIsRegulatedAreaForLoan`(두 컬럼이 모두 확정돼야 규제 적용, 확인필요 시엔 보수적으로 비규제 처리) → `loan-limit.service.js`/`loan-scenario.service.js`의 LTV/DSR/최대대출액 계산까지 그대로 흘러가는 기존 파이프라인은 코드 변경 없이 그대로 재사용했다(DB 값만 정정하면 계산 결과가 자동으로 갱신되는 구조였음). 화성 동탄/용인 기흥/평택 등 다른 지역은 이번 세션에서 재검증하지 않았으므로 추가 정정 대상일 수 있음을 문서에 남겼다. 백엔드 신규/보강 테스트(오피스텔 요율 경계, VAT 필드) 포함 전체 스위트 통과, 프론트엔드 VAT 안내 문구 테스트 추가 |
| v0.39 | 2026-09-10 | **[대출/자금]·[비교셋] 데이터 불일치 근본원인 수정**: (1) **규제 종류 분리**: 신규 마이그레이션(`1788800000000_split-regulation-area-types`)으로 `is_adjustment_target_area`/`is_speculative_overheated_area` 컬럼을 추가하고 `is_regulated_area = (조정대상지역 OR 투기과열지구)`를 DB CHECK 제약으로 강제(값이 어긋날 수 없음, LTV 계산 파이프라인은 기존 `is_regulated_area` 입력을 그대로 재사용해 변경 없음). `ComplexLoanTab`이 조정대상지역/투기과열지구/토지거래허가구역을 각각 독립적으로 "해당/비해당/확인필요"로 표시하도록 변경(기존엔 "규제지역"으로 뭉쳐 표시). (2) **2026-09 LTV 규정 재검증**: 서브에이전트로 금융위/국토부/정책브리핑을 재조사한 결과 기존 LTV 표(무주택-미해당/1주택 규제 40%·비규제 70%, 생애최초 70%/80%)가 2025.10.15 대책까지 반영된 현행 기준과 일치함을 확인, 코드 변경 없음. 사용자가 제시한 "15억원 기준 가격구간 LTV%" 가설은 기존에 이미 구현된 "규제지역 내 주담대 총액 한도"(15억 이하 6억 등, 대출한도 금액이지 LTV%가 아님)와의 혼동으로 판명. (3) **동분당더퍼스트·신흥역하늘채랜더스원1단지(id=49, 성남 중원구 신규 등록 단지) 규제지역 데이터 오류 수정**: id=49는 어제(v0.38) 일괄 정정 이후 새로 등록된 단지라 반영되지 않고 false/null로 남아있었음 — 국토교통부공고 제2025-1219호 재확인 후 세 규제 플래그 모두 true로 정정. (4) **비교셋 "모든 아파트 연식 1년차" 오류 원인 발견·수정**: 진짜 원인은 코드 버그가 아니라 데이터 품질 — 다수 단지의 `completion_year`가 조회년도(2026)로 잘못 채워져 있어 `현재연도-2026+1=1년차`로 계산된 것. 국토교통부 실거래가 API의 `buildYear` 필드로 재검증하는 어제 도입한 `backfill-completion-year-from-molit.js`를 재실행해(일일 호출 한도가 초기화되어 이번엔 39개 단지 전부 성공) 오류값을 실제 준공년도로 정정. 부수적으로 상세 개요(`LocalityAxisList`)가 "연식"을 준공년도만("1998년") 표시하고 비교셋(`buildComparisonRows`)은 경과년수("29년차")를 표시해 두 화면의 정의가 서로 달랐던 것도 발견 — `calcHousingAge`를 `shared/utils/housingAge.ts`로 공용화해 두 화면이 동일한 함수로 "1998년 (29년차)" 형식을 함께 쓰도록 통일. `nearbyRedevelopmentInfo`/`completionYear`가 null일 때의 표시도 "정보 없음"에서 "확인 필요"로 통일(비교셋에 이미 적용된 v0.37 기준과 동일하게 상세 개요도 정정). (5) **버스정류장 API 사용처 확인**: 어제(v0.38 세션) 인증 테스트에는 성공했으나 이를 실제로 호출하는 repository/service 코드가 프로젝트에 전혀 없어 "이미 연동했는데 UI에 반영 안 됨"이 아니라 "연동 자체가 아직 없음"이 정확한 상태임을 확인. 좌표 기반 반경검색 파라미터명이 문서화되어 있지 않아(`gpsLati`/`gpsLong` 시도 시 `MISSING_REQUIRED_PARAMETER`) 정확한 요청 스펙 확인이 더 필요해 이번 버전에서는 연동을 보류하고 §7.1에 미해결 이슈로 기록. 백엔드 53개 스위트·608개 테스트 중 기존에 알려진 무관한 리모델링 날짜-drift 실패 3건 제외 전부 통과, 프론트엔드 신규 회귀 테스트(연식 신축/10년/20년/30년 케이스, 조정대상지역·투기과열지구 독립 표시) 포함 전부 통과 |
