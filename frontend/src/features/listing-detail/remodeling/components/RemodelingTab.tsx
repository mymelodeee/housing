...
import type { RemodelingSource, RemodelingStatus } from '../types'
import './RemodelingTab.css'

interface RemodelingTabProps {
  listingId: string
}
...
function StatusBadge({ status }: { status: RemodelingStatus }) {
  return (
    <Badge variant={status === 'confirmed' ? 'default' : 'needs-confirmation'}>{STATUS_LABEL[status]}</Badge>
  )
}
...
function formatHouseholds(before: number | null, after: number | null, increase: number | null): string {
  if (before === null || after === null) return '정보 없음'
  const base = `${before.toLocaleString()}세대 → ${after.toLocaleString()}세대`
  return increase === null ? base : `${base} (+${increase.toLocaleString()})`
}

export function RemodelingTab({ listingId }: RemodelingTabProps) {
  const { data, isLoading, isError } = useListingRemodeling(listingId)
      ...
      <section className="remodeling-tab__section">
        ...
        {currentStage ? (
          <>
            ...
            <MetaLine
              effectiveDate={currentStage.effectiveDate}
              checkedAt={currentStage.checkedAt}
        ...
        {households ? (
          <>
            <div className="remodeling-tab__row">
              <span className="remodeling-tab__value">
                {formatHouseholds(households.before, households.after, households.increase)}
              </span>
              <StatusBadge status={households.status} />
              <FlagBadges isStale={households.isStale} isConflicted={households.isConflicted} />
            </div>
            <MetaLine
              effectiveDate={households.effectiveDate}
              checkedAt={households.checkedAt}
              source={households.source}
            />
          </>
        ) : (
          <p>정보 없음</p>
      ...
      <section className="remodeling-tab__section">
        ...
        {loanStatus ? (
          <>
            ...
            <MetaLine
              effectiveDate={loanStatus.effectiveDate}
              checkedAt={loanStatus.checkedAt}
              source={loanStatus.source}
            />
          </>
        ) : (
          <p>정보 없음</p>
        ...
        <section className="remodeling-tab__section">
          ...
          {firstContribution && (
            <p className="remodeling-tab__value">
              최근 실거래가{' '}
              {priceLink.recentTransactionPrice === null
                ? '정보 없음'
                : formatPriceKorean(priceLink.recentTransactionPrice)}{' '}
              + 분담금 {formatAmount(firstContribution.amount, firstContribution.unit)} = 총 부담 추정{' '}
              {priceLink.estimatedTotalCost === null
                ? '정보 없음'
                : formatPriceKorean(priceLink.estimatedTotalCost)}
            </p>
          )}
