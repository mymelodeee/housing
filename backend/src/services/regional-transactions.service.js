const regionalTransactionCacheRepository = require('../repositories/regional-transaction-cache.repository');
...
async function searchRecentTransactions({ minPrice, maxPrice, minArea, maxArea, city } = {}) {
  const rows = await regionalTransactionCacheRepository.findByFilters({
...
async function selectCacheEntry(cacheId) {
  const row = await regionalTransactionCacheRepository.findById(cacheId);
  if (!row) return null;
...
// 단지 상세(§14 domain 정리) 진입 전용 — listings row를 만들지 않고 단지만 find-or-create한다.
// 판매 매물이 아니라 실거래 캐시를 보고 단지를 확인하는 흐름이라 sale_price 탐색 범위 체크가 필요 없다.
async function selectCacheEntryComplex(cacheId) {
  const row = await regionalTransactionCacheRepository.findById(cacheId);
  if (!row) return null;

  const coordinates = await resolveCoordinates(row);
  if (!coordinates) {
    const err = new Error('선택한 항목의 좌표를 확인할 수 없습니다');
    err.status = 422;
    throw err;
  }
...
module.exports = { searchRecentTransactions, selectCacheEntry, selectCacheEntryComplex, mapCacheRow };
