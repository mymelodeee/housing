...
function makeGroup(overrides: Partial<LiveListingGroup> = {}): LiveListingGroup {
...
describe('LiveListingComplexGroup', () => {
  it('기본 상태에서는 단지 요약 정보만 보이고 개별 매물 카드는 숨겨진다', () => {
    const { container } = render(<LiveListingComplexGroup group={makeGroup()} onSelect={vi.fn()} />)

    expect(screen.getByText('단지A')).toBeInTheDocument()
    expect(screen.getByText('1건')).toBeInTheDocument()
    expect(container.querySelector('.live-listing-group__meta')).toHaveTextContent('1000세대')
  ...
  it('최소/최대 매매가가 다르면 범위로, 같으면 단일 가격으로 표시한다', () => {
    const { container } = render(
      <LiveListingComplexGroup
        group={makeGroup({ minSalePrice: 90000, maxSalePrice: 90000 })}
        onSelect={vi.fn()}
      />,
    )

    expect(container.querySelector('.live-listing-group__meta')).toHaveTextContent('9억')
  })

  it('헤더를 클릭하면 펼쳐지고 각 매물 카드가 렌더링된다', async () => {
    const user = userEvent.setup()
    const entries = [makeEntry({ id: 1 }), makeEntry({ id: 2 })]
    render(<LiveListingComplexGroup group={makeGroup({ entries })} onSelect={vi.fn()} />)
  ...
  it('펼친 상태에서 매물 카드를 클릭하면 onSelect가 해당 항목의 id로 호출된다', async () => {
    ...
    const { container } = render(<LiveListingComplexGroup group={makeGroup({ entries })} onSelect={onSelect} />)
  ...
  it('주소/세대수 정보가 없으면 안내 문구를 표시한다', () => {
    render(
      <LiveListingComplexGroup group={makeGroup({ address: null, householdCount: null })} onSelect={vi.fn()} />,
    )
