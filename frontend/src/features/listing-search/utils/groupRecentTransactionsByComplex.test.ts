...
import { groupRecentTransactionsByComplex } from './groupRecentTransactionsByComplex'
...
describe('groupRecentTransactionsByComplex', () => {
  it('같은 lawdCd+단지명의 항목을 하나의 그룹으로 묶는다', () => {
    ...
    const groups = groupRecentTransactionsByComplex(entries)
  ...
  it('그룹의 최소/최대 매매가를 계산한다', () => {
    ...
    const [group] = groupRecentTransactionsByComplex(entries)
  ...
  it('빈 배열이면 빈 배열을 반환한다', () => {
    expect(groupRecentTransactionsByComplex([])).toEqual([])
  })
})
