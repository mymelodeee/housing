...
const {
  ...
  collectRegion
} = require('../../scripts/collect-regional-transactions');

describe('scripts/collect-regional-transactions', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('resolveCoordinatesForComplex', () => {
    it('캐시에 이미 좌표가 있으면 geocoding을 호출하지 않는다', async () => {
      regionalTransactionCacheRepository.findCoordinatesByComplexName.mockResolvedValue({ latitude: 37.1, longitude: 127.1 });
    ...
    it('캐시에 좌표가 없으면 address 또는 지역명+단지명으로 geocoding을 호출한다', async () => {
      regionalTransactionCacheRepository.findCoordinatesByComplexName.mockResolvedValue(null);
  ...
  describe('collectRegion', () => {
    it('단지 목록과 실거래 데이터를 매칭하고 기본정보 API로 세대수를 조회해 캐시에 upsert한다', async () => {
      ...
      regionalTransactionCacheRepository.upsertEntry.mockResolvedValue({ id: 1 });

      const count = await collectRegion({ regionName: '화성', lawdCd: '41590' });

      expect(count).toBe(1);
      expect(regionalTransactionCacheRepository.upsertEntry).toHaveBeenCalledWith(
